...
Basic example, generic function worked over a range of types: 
```ts
function identity<Type>(arg: Type): Type {
  return arg;
}
let output = identity("myString"); // the compiler can looked at the value "myString", and set Type to its type.
```
