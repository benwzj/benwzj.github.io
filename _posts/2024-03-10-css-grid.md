...
The `fr` unit make lot of layouts much easier. 
`fr` represents one **fraction** of the available space in the grid container to flexibly size grid rows and columns.
...
```
The `fr` unit can be used with other CSS length such as %, px or em.
...
- [grid playground](https://mozilladevelopers.github.io/playground/css-grid)
